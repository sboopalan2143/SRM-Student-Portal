// ignore_for_file: lines_longer_than_80_chars, avoid_dynamic_calls, unnecessary_await_in_return, no_default_cases
import 'dart:convert';
import 'dart:io';
...
class AppVersionChecker {
  ...
  Future<AppCheckerResult> checkUpdate() async {
    ...
    final currentVersionName = currentVersion ?? packageInfo.version;
    final packageName = appId ?? packageInfo.packageName;
    if (Platform.isAndroid) {
      switch (androidStore) {
        case AndroidStore.apkPure:
          return await _checkApkPureStore(currentVersionName, packageName);
        default:
          return await _checkPlayStore(currentVersionName, packageName);
      }
    } else if (Platform.isIOS) {
      return await _checkAppleStore(currentVersionName, packageName);
    } else {
